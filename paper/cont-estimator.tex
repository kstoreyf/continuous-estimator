...
To complete the connection to least-squares fitting, we also require a normalization matrix; this is constructed from the outer product of the random-random pair projections, for technical reasons described below.
...
\KSF{THIS PARAGRAPH NEEDS HOGG PASS / RED PEN}
...
\KSF{I think we have to say this, and can't just say improved accuracy generally; because now that we're comparing to the binned errors, it's not clear that we're more accurate in any of these examples. An issue is that in our spline experiment which has the same number of components, we are also similar accuracy... but I think if you chose a better basis you could get improvement w same number of components. Do I need to explain this?}
...
This approach would essentially perform a direct estimation of the parameters, without the need for the intermediate steps of binning and fitting; \new{this could be particularly useful for extracting information from small-scale clustering.} 
\KSF{tbh im not 100\% clear on why; chang mentioned this in his email. assume related to the fact that smaller scales have high measurement precision but also high theoretical complexity - but not sure exactly how this direct projection onto derivatives particularly helps here}     
...
\begin{itemize}
    ...
    \item Incorporate other tracer properties into the basis functions, removing the need for binning along other axes;
    ...
    \item Use basis functions that are tailored to the scientific goal, such as directly using the model as bases, avoiding an intermediate fitting step;
    \item Result in correlation functions that are more accurate with fewer components, lowering  requirements on costly mock catalogs.
\end{itemize}
...
%tophat normalization: 1/(sum_nn',k f_k(Gnn')) = (sum_nn',k A_nn'k)^-1
\begin{equation}
...
\end{equation}
and $y$ is a vector of the pair observations.
...
The vector $y$ enumerates the cell pairs, $\bld{y}_{nn'} = \mathcal{N}_n\mathcal{N}_{n'}$, but we only need to consider the cell pairs containing galaxies, so $y$ is simply a vector of ones.
