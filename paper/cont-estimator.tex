...
\begin{abstract}\noindent
% Context / Aims
\KSF{figure out whether should be correlation-function estimator w hyphen! A: i believe no, bc no ambiguity (https://apastyle.apa.org/learn/faqs/when-use-hyphen)}
...
In the standard case, $\GG{ll'}$ would effectively be an imaginary pair of galaxies that has a separation $r$ at which we want to evaluate $\xi$, and we would compute $\bld{\hat{\xi}}$ for such a pair at every separation we are interested in.
