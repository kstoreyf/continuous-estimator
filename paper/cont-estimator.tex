...
We emphasize that here $\GG{ll'}$ does not represent a real pair of galaxies, but instead allows us to evaluate the \cf at any set of parameters.
...
We also note that we do not bold $\hat{\xi}(r)$ or $\hat{\xi}(\GG{ll'})$ as these indicate the evaluation of the \cf at a particular point in parameter space, which outputs a scalar, while in the form $\bld{\hat{\xi}}$ we are representing the full vector output of the estimator.
