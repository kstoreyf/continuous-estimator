...
\KSF{These covariance matrix requirements are also burdensome for weak lensing surveys [CITE]; this work is relevant to these as well.}
...
\KSF{what do you think about the placement of this intuitive description here (the bolded next 2 paragraphs)? makes this section quite long, and a bit of an odd way to introduce it, but not sure where else to put it. and it might make the technical part more understandable, which is what the referee asked for.}
...
\new{We arrive at the formulation of \est by combining the \LS estimator defined above in equations~(\ref{eq:ls1}--\ref{eq:ls3}) with a least-squares fitting approach, detailed in Section~\ref{sec:leastsq}.}
...
\begin{equation}
    \label{eq:xi_proj}
    \hat{\xi}(\GG{ll'}) \equiv \bld{\hat a}\T \cdot \ff(\GG{ll'}) ~,
\end{equation}
where $\GG{ll'}$ contains the data values at which to evaluate $\bld{\hat{\xi}}$, and $\bld{\hat a}$ is a $K$-vector of the computed \emph{amplitudes} of the basis functions
\begin{equation}
    \label{eq:amplitude}
...
\end{equation}
where the $K$ scalar functions $f_k$ are the components of the vector function $\ff$. 
...
Then the evaluation of the amplitudes $\bld{\hat a}$ and the correlation function estimate $\bld{\hat{\xi}}$ results in the equivalent of the \LS estimator--just displayed in a continuous form.
\KSF{audit a to hat a}

\KSF{TODO: add reference to next section}
...
\KSF{TODO: wrap this section with new to bold}

The formulation of \est is based on linear least-squares fitting; in this section we derive the connection between the two.
This derivation is closely connected to the derivation of a similar estimator given by \cite{Tessore2018}.

We can formulate the goal of clustering estimation as finding the best representation of spatial data in the space of two-point separation.
...
We can make this parallel explicit by dividing our survey window into a fine grid of cells (or ``voxels'') such that each cell contains exactly 0 or 1 galaxies. 
...
\KSF{don't love indexing cell pairs by a single value c, but if do something like cc' then indexing X in the following equation gets weird; thoughts?}
These features can be described as the values of basis functions $\ff$ given the properties $\GG{c}$ of the cell pair indexed by $c$ (properties that include, say, the separation $r$ between the cells):
\begin{equation}
    X_{ck} = f_k(\GG{c}) ~.
\end{equation}
For a given galaxy catalog in this survey window, we can define a vector of observables $\bld{y}$ of length $\NN{CC}$.
Where the cell pair hosts a data pair, $y_c = 1$; elsewhere, $y_c = 0$.

With this notation, we can rewrite our projection vectors of equations (\ref{eq:vdd})--(\ref{eq:vrr}) as
\begin{eqnarray}
...
The terms of \eqt{eq:amps_lsq} parallel the definition of generalized least-squares in \eqt{eq:leastsq}.
...
\KSF{change all l to ell}
Each term is the least-squares estimate for the two-point correlation function of the occupation number of the catalog pair, $\langle \mathcal{N}_{\ell} \, \mathcal{N}_{\ell'} \rangle$, where $\ell \ell'$ indexes a cell pair.
We then see that we can predict the occupation number at any cell pair, by defining $X_{ll'}$ as the feature vector of length $K$ of a cell pair $ll'$, by taking the product of this with the amplitudes:
\begin{equation}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, \simeq \, \frac{\NN{CC}}{\NN{DD}} \langle \mathcal{N}_{\mathrm{D},l} \, \mathcal{N}_{\mathrm{D},l'} \rangle 
    - 2\,\frac{\NN{CC}}{\NN{DR}}\,\langle \mathcal{N}_{\mathrm{D},l} \, \mathcal{N}_{\mathrm{R},l'} \rangle
    + \frac{\NN{CC}}{\NN{RR}} \langle \mathcal{N}_{\mathrm{R},l} \, \mathcal{N}_{\mathrm{R},l'} \rangle ~.
\end{equation}
Here we have used the $\simeq$ symbol to indicate that $\bld{X}_{ll'} \, \bld{\hat{a}}$ is an estimator for the quantity on the right-hand side.

We can understand the quantity $\langle \mathcal{N}_{l} \, \mathcal{N}_{l'} \rangle$ as the joint probability of finding galaxies in a pair of cells $l$ and $l'$.
For the random-random cross-correlation which has a vanishing correlation function, this is simply
\begin{equation}
    \langle \mathcal{N}_{\mathrm{R},l} \, \mathcal{N}_{\mathrm{R},l'} \rangle = \frac{\NN{RR}}{\NN{CC}} ~.
\end{equation}
We can make a similar statement for the data-random catalog which has a vanishing correlation function.
For the data-data cross-correlation which has a non-vanishing correlation function $\xi_{ll'}$, this becomes (by the definition of the correlation function)
\begin{equation}
    \langle \mathcal{N}_{\mathrm{D},l} \, \mathcal{N}_{\mathrm{D},l'} \rangle = \frac{\NN{DD}}{\NN{CC}} \left( 1 + \xi_{ll'} \right) ~.
\end{equation}
Plugging these into the amplitude equation, we find
\begin{eqnarray}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, &\simeq& \, \frac{\NN{CC}}{\NN{DD}} \left( \frac{\NN{DD}}{\NN{CC}} \left( 1 + \xi_{ll'} \right) \right)
    - 2\,\frac{\NN{CC}}{\NN{DR}}\,\left( \frac{\NN{DR}}{\NN{CC}} \right)
    + \frac{\NN{CC}}{\NN{RR}} \left( \frac{\NN{RR}}{\NN{CC}} \right) ~. \\
    \label{eq:xi_lsq}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, &\simeq& \, \xi_{ll'} ~.
\end{eqnarray}
We see that our amplitudes $\bld{\hat{a}}$ are a least-squares estimate of the correlation function, where we have assumed that we are working in the limit of a very large random catalog.
Equation (\ref{eq:xi_lsq}) shows that these best-fit amplitudes give the prediction of the correlation function at a new cell pair; this is precisely what we define in \eqt{eq:xi_proj} as the evaluation of the \cf.
...
\KSF{make flow - closely related to kde methods}
\new{Closely related to KDE approaches, methods have been developed to measure the 3-dimensional overdensity field convolved with particular kinds of 3-dimensional kernels, even when only 2-dimensional transverse pair separations have been measured.
These have been used to make measurements of overdensities around individual galaxies \citep{EisensteinXX}, amplitudes of small-scale clustering \citep{PadmanabhanXX}, and the scale of the baryon acoustic feature \citep{XuXX}.}
\KSF{add sentence about similarity in spirit about customizing the functional form to the scientific question of interest ?}
...
This is very different from the KDE approaches, which in most contexts result in biased estimates of the correlation function, because they perform a smoothing.
...
The authors thank Michael Blanton, Jeremy Tinker, Roman Scoccimarro, David Grier, Alex Barnett, \new{Ashley Ross,} Lucia Perez, James Rhoads, Sangeeta Malhotra, \new{Martin White, Nicholas Tessore,} Drew Jamieson, Chris Lovell, and the members of the Flatiron Astronomical Data Group for helpful discussions and feedback.
...
\KSF{i'm pretty sure these f's should not be bolded because i'm evaluating them at r in a bin k; but i had it bolded before so wanted to double check}
