...
\end{equation}
\KSF{i decided to define these because i keep using them. are they ok in-line here or should i make them an equation?}
...
This derivation is closely connected to that given by \cite{Tessore2018}.
...
\begin{equation}
    \label{eq:leastsq}
    \bld{\theta} = [\bld{X}\T\,\bld{C}\inv\,\bld{X}]\inv \, [\bld{X}\T\,\bld{C}\inv\,\bld{y}] ~,
\end{equation}
where $\bld{\theta}$ is the vector of best-fit parameters, $\bld{X}$ is a design matrix containing functions of fitting features, $\bld{C}$ is the covariance matrix, and $\bld{y}$ is a column vector of data to be fit.
...
Each term is the least-squares estimate for the two-point correlation function of the occupation number of the catalog pair, $\langle \mathcal{N}_{l} \, \mathcal{N}_{l'} \rangle$, where $ll'$ indexes a cell pair.
...
\begin{equation}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, \approx \, \frac{\NN{CC}}{\NN{DD}} \langle \mathcal{N}_{\mathrm{D},l} \, \mathcal{N}_{\mathrm{D},l'} \rangle 
...
\end{equation}
Here we have used the $\approx$ symbol to show that $\bld{X}_{ll'} \, \bld{\hat{a}}$ is an estimator for the quantity on the right-hand side. \KSF{overloading $\approx$ here! used for limit and estimate. is there an alternative for one of them?}
...
For the data-data cross-correlation which has a non-vanishing correlation function $\xi_{ll'}$, this becomes
\begin{equation}
...
\begin{eqnarray}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, &\approx& \, \frac{\NN{CC}}{\NN{DD}} \left( \frac{\NN{DD}}{\NN{CC}} \left( 1 + \xi_{ll'} \right) \right)
    - 2\,\frac{\NN{CC}}{\NN{DR}}\,\left( \frac{\NN{DR}}{\NN{CC}} \right)
    + \frac{\NN{CC}}{\NN{RR}} \left( \frac{\NN{RR}}{\NN{CC}} \right) ~. \\
    \label{eq:xi_lsq}
    \bld{X}_{ll'} \, \bld{\hat{a}} \, &\approx& \, \xi_{ll'} ~.
\end{eqnarray}
...
\KSF{should i keep this next bit? i included because Tessore goes out of his way to say that his estimator is unbiased, and ive been asked this in talks, so thought it might be important to clarify here.}
